...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.register_buffer(
            ...
            torch.tril(torch.ones(config.d_block, config.d_block)).view(
                1, config.d_block, config.d_block
            ),
        )
    ...
    def forward(self, x, mask=False):
        ...
        kq = torch.matmul(k, torch.transpose(q, -1, -2)) / math.sqrt(
            self.config.d_model
        )
        if mask:
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.register_buffer(
            "mask",
            torch.tril(
                torch.ones(self.config.d_block, self.config.d_block).view(
                    1, 1, self.config.d_block, self.config.d_block
                )
            ),
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, config: TransformerConfig, decoder: bool = True):
...
class Transformer(nn.Module):
        ...
        self.layers = nn.ModuleDict(
            {
                ...
                "positional_encoding": nn.Embedding(
                    self.config.d_block, self.config.d_model
                ),
                "decoder": nn.ModuleList(
                    [
                        TransformerBlock(self.config, decoder=True)
                        for _ in range(self.config.n_decoders)
                    ]
                ),
    ...
    def configure_optimizers(self, config: TransformerConfig):
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            idx_cond = (
                idx
                if idx.shape[1] <= self.config.d_block
